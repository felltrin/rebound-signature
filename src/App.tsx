...
class Slender {
  ...
  onGameStart() {
    ...
    ammojs_.initialize();
    if (this.test) this.test.ammoJsController = ammojs_;
    ...
    if (this.test && this.test.scene) {
      this.test.scene.background = new THREE.CubeTextureLoader()
        .setPath("/skybox/")
        .load([
          "posx.jpg",
          "negx.jpg",
      ...
      this.test.scene.add(box);
      this.test.scene.add(plane);
    }
  }
  ...
  loadMaterial_(name: string, tiling: number) {
    ...
    const maxAnisotropy = this.test.renderer.capabilities.getMaxAnisotropy();
